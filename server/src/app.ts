...
async function profanityData(link: string) {
    ...
    await fs.writeFile('server/clones/clone.html', data, "utf-8", function (err: ErrnoException | null) {
        if (err) throw err;
    });
...
app.post('/api/profanity', async function (req, res) {
    ...
    return res.sendFile(__dirname + '/clones/screenshot.png')
    // res.sendFile(__dirname + '/clones/clone.html')
})
